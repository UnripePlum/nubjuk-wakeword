...
def write_training_yaml(
    ...
    train_dir: Path,
    adversarial_features_dir: Path | None = None,
    ...
    def has_mmap_features(path: Path) -> bool:
        if not path.exists():
            return False
        return any(path.glob("**/*_mmap/"))
    ...
    if adversarial_features_dir is not None and has_mmap_features(adversarial_features_dir):
        features.append(
            {
                "features_dir": str(adversarial_features_dir),
                "sampling_weight": 8.0,
                "penalty_weight": 1.0,
                "truth": False,
                "truncation_strategy": "random",
    ...
    config["train_summary_step_interval"] = train_summary_step_interval
    config["train_summary_flush_interval"] = train_summary_flush_interval
...
def run_model_train_eval(
    ...
    cmd = [
        sys.executable,
        "-m",
        "mcu_wakeword_engine.model_train_eval",
