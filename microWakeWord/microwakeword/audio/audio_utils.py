...
def generate_features_for_clip(
    ...
    if use_c:
        ...
        while audio_idx + 160 * 2 < num_audio_bytes:
            sample_chunk = audio_samples[audio_idx : audio_idx + 160 * 2]

            # pymicro-features API differs by build/version:
            #   - process_samples (newer snake_case)
            #   - ProcessSamples (older PascalCase)
            if hasattr(micro_frontend, "process_samples"):
                frontend_result = micro_frontend.process_samples(sample_chunk)
            else:
                frontend_result = micro_frontend.ProcessSamples(sample_chunk)
